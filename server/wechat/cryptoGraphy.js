...
CryptoGraphy.prototype.decryptMsg = function(encryptMsg) {
  ...
  //实例 AES 解密对象
  var deCipheriv = crypto.createDecipheriv(
    this.aesModel,
    this.encodingAESKey,
    this.iv
  );
  ...
  var deEncryptedMsg = Buffer.concat([
    deCipheriv.update(encryptMsg, "base64"),
    deCipheriv.final()
  ]).toString("utf8");
  //获取填充字符串的位置
  var pad = deEncryptedMsg.charCodeAt(deEncryptedMsg.length - 1);
  //对微信消息进行处理
  deEncryptedMsg = deEncryptedMsg
    .slice(20, -pad)
    .replace(/<\/xml>.*/, "</xml>");
  //讲解密后的XML 转为 JSON 对象
...
CryptoGraphy.prototype.encryptMsg = function(xmlMsg) {
  ...
  var content =
    ...
    pack;
  //实例 AES 加密对象
  var cipheriv = crypto.createCipheriv(
    this.aesModel,
    this.encodingAESKey,
    this.iv
  );
  ...
  //对明文加密
  var encryptedMsg = Buffer.concat([
    cipheriv.update(content, "binary"),
    cipheriv.final()
  ]).toString("base64");
  //获取认证签名
...
CryptoGraphy.prototype.getMsgSignature = function(encryptedMsg) {
  ...
  var tempStr = [this.token, this.timestamp, this.nonce, encryptedMsg]
    .sort()
    .join("");
  //创建加密类型
